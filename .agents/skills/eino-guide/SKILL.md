...
Implementations live in `eino-ext` (OpenAI, Codex, Gemini, Ark, Ollama, Milvus, Redis, Elasticsearch, etc.).
...
| Package | Contains |
|---------|----------|
| `components/model/{provider}` | ChatModel implementations (openai, Codex, gemini, ark, ollama, deepseek, qwen, etc.) |

...
The system SHALL persist a global enable flag per builtin tool in a `tool_registry` table,
seeded from the builtin registry on startup with a default of enabled. Tool assembly SHALL
exclude any tool whose global enable flag is disabled, then apply the existing per-agent
allowlist, so the effective tool set is the intersection of globally-enabled and
per-agent-allowed tools. Toggling a tool SHALL take effect on subsequent agent runs without a
process restart.
...
#### Scenario: Global enable intersects the per-agent allowlist

- **WHEN** a tool is globally enabled but absent from an agent's allowlist

...
The system SHALL encrypt every stored API key with AES-256-GCM before writing it
to the `config_secrets` table. Each encrypted value SHALL carry its own random salt and
nonce. The database SHALL NOT contain any plaintext API key.
...
- **THEN** the column contains only a base64 blob of `salt ‖ nonce ‖ ciphertext ‖ tag`, never the raw key
...
first, then from the encrypted `config_secrets` table, then error. An env-provided key
...
#### Scenario: Missing key produces a guided error

- **WHEN** no env var and no stored secret exist for the selected provider
...
Each profile SHALL have a `settings` JSON field (default `{}`) for
provider-specific extras and an `enabled` flag (default true). A disabled profile
SHALL NOT be selectable as a provider until re-enabled.
